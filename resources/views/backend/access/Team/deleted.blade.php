...
@section ('title', trans('labels.backend.access.Team.management') . ' | ' . trans('labels.backend.access.Team.deleted'))

@section('page-header')
    <h1>
        {{ trans('labels.backend.access.Team.management') }}
        <small>{{ trans('labels.backend.access.Team.deleted') }}</small>
    </h1>
@endsection
...
@section('content')
    <div class="box box-info">
        <div class="box-header with-border">
            <h3 class="box-title">{{ trans('labels.backend.access.Team.deleted') }}</h3>
        ...
        <div class="box-body">
            <div class="table-responsive data-table-wrapper">
                <table id="users-table" class="table table-condensed table-hover table-bordered">
                    <thead>
                        <tr>
                            <th>{{ trans('labels.backend.access.Team.table.Team_name') }}</th>
                    ...
                    <thead class="transparent-bg">
                        <tr>
                            <th>
                                {!! Form::text('Team_name', null, ["class" => "search-input-text form-control", "data-column" => 0, "placeholder" => trans('labels.backend.access.Team.table.Team_name')]) !!}
	...
	<script>
            ...
            (function () {
                ...
                var dataTable = $('#users-table').dataTable({
                    ...
                    serverSide: true,
                    ajax: {
                        url: '{{ route("admin.access.Team.get") }}',
                        type: 'post',
                        data: {status: false, trashed: true}
                    },
                    columns: [
                        {data: 'Team_name', name: '{{config('access.users_table')}}.Team_name'},
